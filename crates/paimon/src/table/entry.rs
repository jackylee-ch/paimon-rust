...
pub(super) fn bitmap_meta_may_match(
    ...
    or_with_alternate_zeros(data_type, serialized_literals, |literals| {
        meta.may_match(op, literals, cmp)
    })
}

/// Run `check` on the literals, then again on just their opposite-signed-zero
/// counterparts.
///
/// A `±0.0` literal also matches rows holding the other signed zero, but the
/// comparator orders `-0.0` below `+0.0`, so a key range that stops at one of them
/// would prune a file the reader would have matched. The alternates are a call of
/// their own rather than extra literals because `BTreeIndexMeta::may_match` reads
/// only the first literal for `Eq`.
fn or_with_alternate_zeros(
    data_type: &DataType,
    serialized_literals: &[Vec<u8>],
    check: impl Fn(&[Vec<u8>]) -> bool,
) -> bool {
    if check(serialized_literals) {
        return true;
    }
    let alternate_zeros: Vec<Vec<u8>> = serialized_literals
        ...
        .collect();
    !alternate_zeros.is_empty() && check(&alternate_zeros)
}
...
/// Array membership is compared *bitwise* at the row level (`arrow::residual`
/// matches on `to_bits()`, mirroring Java's `Float.compareTo` in
/// `CompareUtils.compareLiteral`), so `-0.0` and `+0.0` are genuinely different
/// elements here and this check must not widen them the way the scalar path does.
...
mod tests {
    ...
    fn assert_signed_zero_survives_meta_prune(
        ...
        // The multivalue kind serves the array operators, whose row-level filter
        // compares elements bitwise, so it must keep pruning a -0.0-only file for
        // a +0.0 literal -- those elements are not occurrences of +0.0.
        let meta = BTreeIndexMeta::new(
            Some(negative_zero_key.clone()),
            Some(negative_zero_key.clone()),
            false,
        ...
        for op in [
            ...
            assert!(
                !multivalue_meta_may_match(
                    &meta,
                    op,
                    std::slice::from_ref(&positive_zero_key),
                    &cmp
                ),
                "{op} must stay exact for signed zeros ({data_type:?})"
            );
        }
